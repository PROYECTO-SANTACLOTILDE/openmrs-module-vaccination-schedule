...
public class PatientVaccinationSchedule extends BaseOpenmrsData {
	...
	public PatientVaccinationSchedule(Patient patient, VaccinationSchedule schedule, User assignedByUser) {
		this();
		if (patient == null) {
	...
	public List<VaccinationScheduleEntry> getOverdueVaccinations() {
		...
		return entries.stream()
		        .filter(
		            entry -> entry != null && entry.getAgeInDaysMax() != null && currentAgeInDays > entry.getAgeInDaysMax())
	...
	public List<VaccinationScheduleEntry> getUpcomingVaccinations() {
		...
		return entries.stream()
		        .filter(
		            entry -> entry != null && entry.getAgeInDaysMin() != null && currentAgeInDays >= entry.getAgeInDaysMin()
	...
	public String toString() {
		return "PatientVaccinationSchedule{" + "id=" + id + ", patient="
		        + (patient != null ? patient.getPatientId() : "null") + ", vaccinationSchedule="
		        + (vaccinationSchedule != null ? vaccinationSchedule.getName() : "null") + ", assignedDate=" + assignedDate
		        + ", status=" + status + '}';
	}
}
